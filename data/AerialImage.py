...
#         image_selected, mask_selected = random.choice(list(zip(image_crops_all, mask_crops_all)))
    ...
    def load_data(self):
        all_images = []
        all_masks = []
        for current_dataset in self.available_datasets:
    ...
    def apply_augmentations(self, image, mask, prob=0.5):
        ...
        if random.random() < prob:
            ...
            image = transformsF.rotate(image, angle, transforms.InterpolationMode.BILINEAR, False, None, fill=0)
            mask = transformsF.rotate(mask, angle, transforms.InterpolationMode.BILINEAR, False, None, fill=255)
        if random.random() < prob:
...
if __name__ == '__main__':
    data = AerialImage('train', '/content/drive/MyDrive/csc413/unext/dataset', ['berlin', 'chicago'], (256, 256))
    print(len(data))
    for image, label in data:
        # print(image.shape, label.shape)
        plt.imshow(image.squeeze().cpu().detach().permute(1, 2, 0))
        break
