...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_type', default='convnext_xlarge_in22k', 
    ...
    if args.model == 'UNext':
        model = UNeXt(3, args.model_type)
    else:
        model = UNetWithResnet50Encoder(n_classes=3)
    ...
    if args.loss == "CrossEntropyLoss":
        ...
        loss_fn = FocalLoss(alpha=0.5, gamma=args.gamma, reduction='mean').to(DEVICE)
    else:
    ...
    # Create datasets
    train_dataset = AerialImage("train", available_datasets=['berlin', 'chicago'], resized_shape=resized_shape, resample_size=resample_size, root_dir=ROOT_DIR)
    val_dataset = AerialImage("val", available_datasets=['berlin', 'chicago'], resized_shape=(2*resized_shape[0], 2*resized_shape[1]), root_dir=ROOT_DIR)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        if iou_road >= iou_road_best:
            ...
            torch.save(model.state_dict(), f"{args.model}_{args.model_type}.pth")
            counter = 0
